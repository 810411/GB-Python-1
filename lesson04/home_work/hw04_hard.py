...
error_msg = 'Введенные данные некорректны!'
...
def withdraw_money(person, money):
    if money < 0:
        return ('Вы ввели отрицательную сумму')
...
def start():
    input_data = input('Введите номер карты и пин код через пробел:')
    try:
        card_number, pin_code = input_data.split()
